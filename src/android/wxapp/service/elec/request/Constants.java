package android.wxapp.service.elec.request;

public class Constants {
	// 请求成功
	public static final int LOGIN_SUCCESS = 100;
	// 请求失败
	public static final int LOGIN_FAIL = 101;

	public static final int LOGIN_UPDATE_SUCCESS = 102;
	public static final int LOGIN_UPDATE_FAIL = 103;
	public static final int LOGIN_UPDATE_SAVE_FAIL = 104;

	public static final int CREATE_TASK_SUCCESS = 105;
	public static final int CREATE_TASK_FAIL = 106;
	public static final int CREATE_TASK_SAVE_FAIL = 107;

	public static final int CREATE_INS_SUCCESS = 108;
	public static final int CREATE_INS_FAIL = 109;
	public static final int CREATE_INS_SAVE_FAIL = 110;

	public static final int UPLOAD_TASK_ATT_SUCCESS = 111;
	public static final int UPLOAD_TASK_ATT_FAIL = 112;
	public static final int UPLOAD_TASK_ATT_SAVE_FAIL = 113;

	public static final int START_TASK_SUCCESS = 114;
	public static final int START_TASK_FAIL = 115;
	public static final int START_TASK_SAVE_FAIL = 116;

	public static final int END_TASK_SUCCESS = 117;
	public static final int END_TASK_FAIL = 118;
	public static final int END_TASK_SAVE_FAIL = 119;

	public static final int DELETE_TASK_SUCCESS = 120;
	public static final int DELETE_TASK_FAIL = 121;
	public static final int DELETE_TASK_SAVE_FAIL = 122;
	
	public static final int MQTT_UPDATE_SUCCESS=123;
	public static final int MQTT_UPDATE_FAIL=124;
}
